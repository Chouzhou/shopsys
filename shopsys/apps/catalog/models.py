...
class Category(models.Model):
    ...
    slug = models.SlugField(
        "Slug",
        max_length=50,unipue=True,
        help_text='根据name生成的，用于生成页面URL，必须唯一'，
    )
    description = models.TextField("描述")
    is_acitve = models.BooleanField("是否激活",default=True)
    ...
    def get_absolute_url(self):
        return reverse('catalgo_category',args=(self.slug,))
class Product(models.Model):
    ...
    old_price = models DecimalField("旧价格",max_digits=9,decimal_places=2,blank=True,
                                        default=0.00
                                    )
    image = models.ImageField("图片",max_length=50)
    is_active = models.BooleanField("设为激活",dafault=True)
    is_bestseller = models.BooleanField("设为推荐",default=False)
